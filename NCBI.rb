class NCBI
  ...
  def self.get_fasta_sequence_of_gi(gi, from='begin', to='end')
    fasta = Net::HTTP.get_response('eutils.ncbi.nlm.nih.gov', "/entrez/eutils/efetch.fcgi?db=nucleotide&id=#{gi}&rettype=fasta&seq_start=#{from}&seq_stop=#{to}")
    return fasta.read_body
  end
